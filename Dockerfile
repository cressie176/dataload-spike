...
# Build inputs: migrations (Drizzle-generated) + the generated seed chunks
# (one independent COPY file per park, loaded in parallel by build-db.sh).
WORKDIR /work
COPY migrations/ /work/migrations/
COPY chunks/ /work/chunks/
...
RUN chmod +x /work/build-db.sh && /work/build-db.sh && rm -rf /work/chunks
